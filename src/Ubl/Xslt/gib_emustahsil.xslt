					...
					<table style="border-color:blue; " border="0" cellspacing="0px" width="800"
						...
						<tbody>
							...
							<tr style="height:118px; " valign="top">
								...
								<div style="text-align:center;height:100px;"><center>
								<div id="qrcode"></div>
...
"odenecek":<xsl:value-of select="n1:CreditNote/cac:LegalMonetaryTotal/cbc:PayableAmount"/>}
				...
				<table id="budgetContainerTable" width="800px">
					<tr align="right">
						...
						<xsl:call-template name="para_okuma"><xsl:with-param name="tutar" select="n1:CreditNote/cac:LegalMonetaryTotal/cbc:PayableAmount"/></xsl:call-template>											
						</div>
						</td>
...
<xsl:template name="para_okuma">
    ...
    <xsl:call-template name="para_okuma_oku" ><xsl:with-param name="tutar"  select="$okunacak" /></xsl:call-template>
    <xsl:if test="$doviz2">
			 <xsl:choose>
				<xsl:when test="$doviz2 =  'TRL' or $doviz2 =  'TRY'">
				   ...
				   <xsl:if test="$noktadan_sonra &gt; 0">
					   <xsl:value-of select="' '" />               
					   <xsl:call-template name="para_okuma_oku" ><xsl:with-param name="tutar"  select="$noktadan_sonra" /></xsl:call-template>
...
<xsl:template name="para_okuma_oku">
    <xsl:param name="tutar" />
	...
	<xsl:if test="$milyarlar &gt; 0">
	   <xsl:call-template name="para_okuma_sayi_oku_3hane" ><xsl:with-param name="sayi"  select="$milyarlar" /></xsl:call-template> Milyar 
	</xsl:if>
	<xsl:if test="$milyonlar &gt; 0">
	   <xsl:call-template name="para_okuma_sayi_oku_3hane" ><xsl:with-param name="sayi"  select="$milyonlar" /></xsl:call-template> Milyon 
	</xsl:if>
	<xsl:if test="$binler &gt; 0">
	    <xsl:if test="$binler = 1">Bin </xsl:if>
	    <xsl:if test="$binler > 1">
	         <xsl:call-template name="para_okuma_sayi_oku_3hane" ><xsl:with-param name="sayi"  select="$binler" /></xsl:call-template> Bin 
        </xsl:if>
	</xsl:if> 

    <xsl:call-template name="para_okuma_yuzler_oku" ><xsl:with-param name="sayi"  select="$yuzler" /></xsl:call-template>
    <xsl:call-template name="para_okuma_onlar_oku" ><xsl:with-param name="sayi"  select="$onlar" /></xsl:call-template>    
    <xsl:call-template name="para_okuma_birler_oku" ><xsl:with-param name="sayi"  select="$birler" /></xsl:call-template>
</xsl:template>
<xsl:template name="para_okuma_sayi_oku_3hane">
     <xsl:param name="sayi" />
...
<xsl:template name="para_okuma_birler_oku">
   <xsl:param name="sayi" />
   <xsl:choose>
...
<xsl:template name="para_okuma_onlar_oku">
   <xsl:param name="sayi" />
   <xsl:choose>
...
<xsl:template name="para_okuma_yuzler_oku">
   <xsl:param name="sayi" />
   <xsl:choose>
...
<xsl:template name="para_okuma_binler_oku">
   <xsl:param name="sayi" />
   <xsl:choose>
...
<xsl:template name="para_okuma_onbinler_oku">
   <xsl:param name="sayi" />
   <xsl:if test="$sayi &gt; 0">
      <xsl:call-template name="para_okuma_onlar_oku" ><xsl:with-param name="sayi"  select="$sayi" /></xsl:call-template>Bin 
   </xsl:if>
</xsl:template>	
</xsl:stylesheet>