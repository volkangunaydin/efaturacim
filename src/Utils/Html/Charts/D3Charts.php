...
class D3Charts extends ChartsBase
{
    ...
    public function toHtmlAsString($doc = null)
    {
        ...
        $html .= '.d3-chart .legend-item { display: inline-block; margin: 5px 10px; cursor: pointer; transition: all 0.3s ease; padding: 4px 8px; border-radius: 4px; }' . "\n";
        $html .= '.d3-chart .legend-item:hover { transform: scale(1.05); background-color: rgba(0,0,0,0.05); }' . "\n";
        $html .= '.d3-chart .legend-item.disabled { opacity: 0.3; text-decoration: line-through; background-color: rgba(0,0,0,0.1); }' . "\n";
    ...
    public function getJsLines()
    {
        ...
        '            .padding(0.2);',
            ...
            '                ',
            '                // Get category name directly from the data object',
            '                var categoryName = actualData && actualData.data ? actualData.data.category : "Unknown";',
            '                ',
            '                ',
