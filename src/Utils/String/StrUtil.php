...
class StrUtil
{
    ...
    public static function removeTurkishChars($str)
    {
        if (is_array($str)) {
            $res = array();
            foreach ($str as $k => $v) {
                $res[$k] = self::removeTurkishChars($v);
            }
            return $res;
        } else {
            return str_replace(array("Ğ", "Ü", "Ş", "İ", "Ö", "Ç", "ğ", "ü", "ş", "ı", "ö", "ç", "Ä", "ä", "Ё", "ё"), array("G", "U", "S", "I", "O", "C", "g", "u", "s", "i", "o", "c", "A", "a", "E", "e"), "" . $str);
        }
    }
    public static function toLowerTurkish($input)
    {
        if (is_array($input)) {
            ...
            foreach ($input as $k => $v) {
                $arr[$k] = self::toLowerTurkish($v);
            }
            return $arr;
        } else {
            return mb_strtolower(str_replace(array("Ğ", "Ü", "Ş", "I", "İ", "Ö", "Ç"), array("ğ", "ü", "ş", "ı", "i", "ö", "ç"), "" . $input), "UTF-8");
        }
    }
    public static function toUpperTurkish($input)
    {
        if (is_array($input)) {
            $arr = array();
            foreach ($input as $k => $v) {
                $arr[$k] = self::toUpperTurkish($v);
            }
            return $arr;
        } else {
            return mb_strtoupper(str_replace(array("ğ", "ü", "ş", "ı", "i", "ö", "ç"), array("Ğ", "Ü", "Ş", "I", "İ", "Ö", "Ç"), "" . $input), "UTF-8");
        }
    }
    ...
    public static function endsWith($str, $endText)
    {
    ...
    public static function trimNewLines($strIn, $newLineReplacement = "")
    {
    ...
    public static function onlyNumeric($str, $canBeNegative = false)
    {
        $unwantedChars = null;
        return self::onlyChars($str, ($canBeNegative ? "0123456789-" : "0123456789"), $unwantedChars);
    }
    ...
    public static function onlyChars($str, $onlyChars, &$unwantedChars)
    {
        if ($str && is_array($str)) {
            $ret = array();
            foreach ($str as $k => $v) {
                $ret[$k] = self::onlyChars($v, $onlyChars, $unwantedChars);
            }
            return $ret;
        }
        $return = "";
    ...
    public static function getShortGuid($useSeperator = true, $minSize = 0, $maxValForRandom = 0, $asShortAsPossible = false)
    {
        if ($maxValForRandom && $maxValForRandom >= 1000) {
            $maxVal = $maxValForRandom;
        } else {
            $maxVal = min(9999999999, getrandmax());
        }
        $time = time();
        $alpha = strtoupper(base_convert($time, 10, 35));
        $rndVal = rand(0, $maxVal);
        $rnd1 = strtoupper(base_convert($rndVal, 10, 35));
        if ($asShortAsPossible) {
            $rnd1 .= "";
        } else if (strlen($rnd1) < 10) {
            $rnd1 = substr("000000000" . $rnd1, -10);
        }
        //$dec  = base_convert(strtolower($alpha), 35, 10);
        $guid = $alpha . ($useSeperator ? "-" : "") . $rnd1;
        if ($minSize > 0 && strlen($guid) < $minSize) {
            $guid = substr("0000000000000000000000000000000000" . $guid, -1 * $minSize);
        }
        return $guid;
    }
    ...
    public static function parse($str, $arr, $smart = true, $esc = false, $recursive = false, $depth = 0)
    {
        ...
        if (is_array($str)) {
            ...
            foreach ($str as $kk => $vv) {
                $t[$kk] = self::parse($vv, $arr);
            }
            return $t;
        }
        if (is_scalar($str) && $smart && key_exists($str, $arr)) {
            return @$arr[$str];
        }
        if ($esc) {
            $escSearch = array("{ ");
    ...
    public static function toVarName($str, $defVal = null, $typeOptions = null)
    {
        ...
        if (Options::ensureParam($typeOptions) && $typeOptions instanceof Options) {
            ...
            if (V::isEmptyString($strVar) && $typeOptions->getAsBool("force")) {
                $strVar = "vnone";
            }
            if ($toLower) {
            ...
            if ($doNotStartWithNumber && self::startsWithNumber($strVar)) {
                $strVar = "v_" . $strVar;
            }
            if (strlen($strVar) > 0) {
                $ret = $strVar;
            }
            if ($maxLength > 0 && strlen($ret) > $maxLength) {
                $ret = substr($ret, 0, $maxLength);
            }
            if (strlen($strVar) > 2 && substr($strVar, -1) == "_") {
                $ret = substr($ret, 0, -1);
            }
