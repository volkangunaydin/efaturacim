...
use Efaturacim\B4B\Laravel\Customization\MyLaravelCustomization;
...
class OrkestraSoapServiceBase
{
    ...
    protected function initMe()
    {
        ...
        if (StrUtil::isEmpty($host) && LV::isLaravel()) {
            ...
            $conf = MyLaravelCustomization::getAsArray(null, [], "orkestra");
            if (is_array($conf) && count($conf) > 0 && key_exists("host", $conf)) {
                foreach ($conf as $key => $val) {
                    $this->options->setValue($key, $val);
                }
                $host = $this->options->getAsString("host");
            } else if (is_array($conf) && count($conf) > 0 && key_exists("default", $conf) && is_array($conf["default"]) && key_exists("host", $conf["default"])) {
                foreach ($conf["default"] as $key => $val) {
                    $this->options->setValue($key, $val);
                }
            }
        }
